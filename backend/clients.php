<?php
// CORS HEADERS (sempre antes de tudo)
...
// Função simples para checar autenticação
function checkAuth() {
...
if ($method === 'GET') {
    ...
    if (!isset($data['id'], $data['nome'])) {
        http_response_code(400);
        echo json_encode(['error' => 'Id ou nome do clioente não encontrado']);
        exit;
    }
    ...
    if (!$id) {
        http_response_code(400);
        echo json_encode(['error' => 'ID do cliente não encontrado']);
        exit;
    }
